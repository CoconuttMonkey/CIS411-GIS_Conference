...
class Download extends Main_Controller {
    ...
    // Set column names
		$fields = (
			$field_array[] = array ("ID", "Contact Email", "Company Name", "Company Address", "URL", "Sponsor Level", "Paid")
		);
		
		// Create array of data
		foreach ($query as $row)
		{
			$data_array[] = array( $row->sponsor_id, $row->main_contact, $row->company_name, $row->company_address, $row->url, $row->sponsor_level, $row->paid );
		}
