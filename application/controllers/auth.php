...
class Auth extends CI_Controller {
	...
	function dashboard()
	{
		...
		else
		{
			...
			$data["unpaid_sponsor_count"]	= count($this->data['unpaid_sponsors'] 	= $this->sponsor_model->get_all($current_conf, 'unpaid'));
			$data["paid_sponsor_count"]		= count($this->data['paid_sponsors'] 		= $this->sponsor_model->get_all($current_conf, 'paid'));
			$data['message'] 											= (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	    ...
	    $this->load->view('include/header');
	    $this->load->view('templates/menubar');
	    $this->load->view('auth/dashboard-admin',$data);
	    $this->load->view('include/footer');
		}
	}
	
	//admin pages
	function users($filter = NULL)
	{
		if (!$this->ion_auth->logged_in())
		{
			//redirect them to the login page
